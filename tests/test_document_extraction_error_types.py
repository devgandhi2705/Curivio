"""
Phase P — document_extraction_service now returns (text, error, error_type),
...
class TestPdfSuccess:
    def test_real_pdf_extracts_text_no_error(self):
        text, error, error_type = des.extract_document_text(_real_text_pdf_bytes(), "r.pdf", ".pdf")
...
class TestPdfFailureReasons:
    def test_corrupt_pdf_bytes(self):
        text, error, error_type = des.extract_document_text(b"not a pdf at all", "bad.pdf", ".pdf")
        assert text is None
        assert error_type == des.ERROR_PDF_UNREADABLE
        assert "bad.pdf" in error

    def test_zero_page_pdf(self):
        text, error, error_type = des.extract_document_text(_zero_page_pdf_bytes(), "empty.pdf", ".pdf")
        assert text is None
        assert error_type == des.ERROR_PDF_NO_PAGES

    def test_scanned_image_only_pdf(self):
        text, error, error_type = des.extract_document_text(_blank_pdf_bytes(), "scan.pdf", ".pdf")
        assert text is None
...
class TestDocxReasons:
    def test_real_docx_extracts_text_no_error(self):
        text, error, error_type = des.extract_document_text(_real_text_docx_bytes(), "r.docx", ".docx")
        assert error is None
        assert error_type is None
        assert "Real extractable docx paragraph." in text

    def test_corrupt_docx_bytes(self):
        text, error, error_type = des.extract_document_text(b"not a docx at all", "bad.docx", ".docx")
        assert text is None
        assert error_type == des.ERROR_DOCX_UNREADABLE

    def test_empty_docx_no_text(self):
        text, error, error_type = des.extract_document_text(_empty_docx_bytes(), "blank.docx", ".docx")
        assert text is None
...
class TestPlainTextReasons:
    def test_real_text_extracts_no_error(self):
        text, error, error_type = des.extract_document_text(b"real content here", "r.txt", ".txt")
        assert error is None
        assert error_type is None
        assert text == "real content here"

    def test_whitespace_only_text_is_empty(self):
        text, error, error_type = des.extract_document_text(b"   \n\t  ", "blank.txt", ".txt")
        assert text is None
...
class TestUnsupportedExtension:
    def test_unknown_extension(self):
        text, error, error_type = des.extract_document_text(b"whatever", "f.xyz", ".xyz")
        assert text is None
